...
from app.repositories.anomaly_repository import AnomalyListQueryParams
...
def anomalies_summary(db: Session = Depends(get_db), _=Depends(get_current_identity)) -> ApiResponse[AnomalySummary]:
    return {"data": AnomalyReadService(db).get_summary()}
...
def get_anomaly(anomaly_id: UUID, db: Session = Depends(get_db), _=Depends(get_current_identity)) -> ApiResponse[AnomalyDetail]:
...
def get_anomaly_investigation(anomaly_id: UUID, db: Session = Depends(get_db), _=Depends(get_current_identity)) -> ApiResponse[AnomalyInvestigation]:
...
def get_latest_analysis(anomaly_id: UUID, db: Session = Depends(get_db), _=Depends(get_current_identity)) -> ApiResponse[AnomalyAnalysisRecord]:
...
def get_analysis(analysis_id: UUID, db: Session = Depends(get_db), _=Depends(get_current_identity)) -> ApiResponse[AnomalyAnalysisRecord]:
...
def list_tool_calls(analysis_id: UUID, db: Session = Depends(get_db), _=Depends(get_current_identity)) -> ApiResponse[list[AnomalyToolCallItem]]:
