...
def summary(filters: Filters = Depends(common_filters), db: Session = Depends(get_db), _=Depends(get_current_identity)) -> ApiResponse[DashboardSummary]:
...
def kpi_summary(filters: Filters = Depends(common_filters), db: Session = Depends(get_db), _=Depends(get_current_identity)) -> ApiResponse[list[KpiSummaryItem]]:
...
def shift_comparison(filters: Filters = Depends(common_filters), db: Session = Depends(get_db), _=Depends(get_current_identity)) -> ApiResponse[list[ShiftComparisonItem]]:
    service = DashboardComparisonService(db)
...
def performance_distribution(filters: Filters = Depends(common_filters), db: Session = Depends(get_db), _=Depends(get_current_identity)) -> ApiResponse[PerformanceDistributionResponse]:
    service = DashboardForemanService(db)
...
def performance_leaders(db: Session = Depends(get_db), _=Depends(get_current_identity)) -> ApiResponse[PerformanceLeadersResponse]:
    service = PerformanceLeadersService(db)
    return {"data": service.leaders()}
